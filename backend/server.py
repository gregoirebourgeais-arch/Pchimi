...
@api_router.get("/recipes/template/xlsx")
async def download_recipe_template_xlsx():
    template = pd.DataFrame([
        {
            "name": "Pancakes",
            "category": "Petit-déjeuner",
            "ingredients": "Farine|200|g;Lait|300|ml;Oeuf|2|pièce",
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="template-recettes.xlsx"'},
    )
...
async def get_grocery_list_pdf(
    week_start: date = Query(..., description="Lundi de la semaine"),
    checklist: bool = Query(False, description="Ajoute des cases à cocher"),
):
    sections = compute_grocery_sections(week_start)
    lines: list[str] = []
    for section in sections:
        ...
        for item in section.items:
            ...
            lines.append(f"{prefix}{item.name}: {item.quantity} {item.unit}")
        lines.append("")
